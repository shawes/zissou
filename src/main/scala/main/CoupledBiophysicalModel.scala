...
class CoupledBiophysicalModel(val config: Configuration) {
  ...
  val ocean = new PhysicalModel(flow, config.inputFiles, randomNumbers)
  val biology = new BiologicalModel(config.fish, config.spawn, config.inputFiles, config.habitat, clock, randomNumbers)
