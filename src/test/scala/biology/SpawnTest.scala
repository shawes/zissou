...
class SpawnTest extends FlatSpec {
  ...
  spawningLocations += new SpawningLocation("lizard island", 1000, 202, new GeoCoordinate(1, 2), releasePeriod1, 1)
  spawningLocations += new SpawningLocation("bare island", 576, 199, new GeoCoordinate(4, 7), releasePeriod2, 1)
  ...
  it should "return first site where it is spawning" in {
    val spawn = new Spawn(spawningLocations.toList)
    val sites = spawn.getSitesWhereFishAreSpawning(date.minusDays(3).toLocalDateTime)
  ...
  it should "return second site where it is spawning" in {
    val spawn = new Spawn(spawningLocations.toList)
    val sites = spawn.getSitesWhereFishAreSpawning(date.plusDays(4).toLocalDateTime)
    assert(releasePeriod2.contains(date))
    assert((releasePeriod1.getStart() to date.toDateTime(DateTimeZone.UTC)).toPeriod.getDays % 5 == 0)
    assert(sites.size == 1)
  }
